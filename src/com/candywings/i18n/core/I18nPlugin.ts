...
export default class I18nPlugin extends Phaser.Plugins.ScenePlugin
  implements Ii18n {
  public static Events: Ii18nPluginEvents = {
  ...
  public boot(): void {
    ...
    this.on(I18nPlugin.Events.LANGUAGE_CHANGE, this.languageChangedBound);
  }
  ...
  public initialize(options: any, callback?: i18next.Callback): void {
    i18next.use(new I18NextXhrBackend(null, options));
    if (options) {
  ...
  public exists(
    key: string | string[],
    options?: i18next.InterpolationOptions,
  ): boolean {
  ...
  private shutdown(): void {
    this.off(I18nPlugin.Events.LANGUAGE_CHANGE, this.languageChangedBound);
    const eventEmitter: Phaser.Events.EventEmitter = this.systems.events;
